...
def predict_prob(classifiers, X):
	...
	"""
	#weight=[0.0]*len(classifiers[1].weight)
	p=0.0
	for i in range(0,len(classifiers)):
		#print 'i' + str(i)
		# for j, v in X:
			# #print 'j' + str(j)
			# weight[j]+=classifiers[i].weight[j]/len(classifiers) 

		p += 1.0 / (1.0 + math.exp(-math.fsum([classifiers[i].weight[f]*v for f, v in X])))
	#p = 1.0 / (1.0 + math.exp(-math.fsum([weight[f]*v for f, v in X])))
	
	return p/len(classifiers)
